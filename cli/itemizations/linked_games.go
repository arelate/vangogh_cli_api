...
func linkedGames(modifiedAfter int64) (vangogh_data.IdSet, error) {
	...
	if len(modifiedApv2) > 0 {
		nod.Log("modified %s: %v", vangogh_data.ApiProductsV2, modifiedApv2)
	}
	...
	for _, id := range modifiedApv2 {
		...
		if len(giiig) > 0 {
			nod.Log("%s #%s is-included-in-games: %v", vangogh_data.ApiProductsV2, id, giiig)
		}
		...
		if len(girbg) > 0 {
			nod.Log("%s #%s is-required-by-games: %v", vangogh_data.ApiProductsV2, id, girbg)
		}
