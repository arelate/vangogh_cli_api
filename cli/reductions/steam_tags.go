...
func SteamTags(since int64) error {
	...
	vrSteamStorePage, err := vangogh_local_data.NewReader(vangogh_local_data.SteamStorePage, gog_integration.Game)
	if err != nil {
		return sta.EndWithError(err)
	}
	...
	for _, id := range modified {
		...
		tagsContainer := match_node.Match(steamStorePage, matchAppTag)
...
func matchAppTag(node *html.Node) bool {
