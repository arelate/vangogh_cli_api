...
func HandleFuncs() {
	v1PatternHandlers := map[string]http.Handler{
		...
		"/v1/digest":    IfReduxModifiedSince(nod.RequestLog(http.HandlerFunc(GetDigest))),
		"/v1/downloads": nod.RequestLog(http.HandlerFunc(GetDownloads)),
		"/v1/keys":      nod.RequestLog(http.HandlerFunc(GetKeys)),
		"/v1/redux":     IfReduxModifiedSince(nod.RequestLog(http.HandlerFunc(GetRedux))),
		"/v1/search":    IfReduxModifiedSince(nod.RequestLog(http.HandlerFunc(Search))),
